...
def make_duplicate_collection(collection: bpy.types.Collection):
    target_collection_name = collection.name + "_copy"
    ...
    if target_collection is None:
        ...
        make_duplicate_objects(collection)

def make_duplicate_objects(collection: bpy.types.Collection):
    """Duplicates objects in collection"""
    target_collection = bpy.data.collections[collection.name + "_copy"]
    for obj in collection.objects:
        new_obj = bpy.data.objects.new(obj.name + "_copy", obj.data.copy())
        new_obj.location = obj.location
        new_obj.rotation_euler = obj.rotation_euler
        new_obj.scale = obj.scale
        target_collection.objects.link(new_obj)

def create_materials(collection: bpy.types.Collection):
    number_of_objects = len(collection.objects)
    
    for material in bpy.data.materials:
        if material.name[0:8] == "Emmision":
            material.user_clear()
            bpy.data.materials.remove(material)
...
def materialize_duplicated_objects(collection: bpy.types.Collection):
    target_collection = bpy.data.collections[collection.name + "_copy"]
    for i, obj in enumerate(target_collection.objects):
        obj.data.materials.clear()
        obj.data.materials.append(bpy.data.materials["Emmision " + str(i)])

def object_location_new(collection: bpy.types.Collection, max_offset: int):
    """Foreach object in collection define object's location"""
    target_collection = bpy.data.collections[collection.name + "_copy"]
    angles = []
    #xi = []
    ...
    for i in range(number_of_objects):
        ...
        #xi.append(x)
        #yi.append(y)
        collection.objects[i].location = x,y,collection.objects[i].location.z
        target_collection.objects[i].location = x,y,target_collection.objects[i].location.z
        """
    for i, obj in enumerate(collection.objects):
        obj.location = xi[i],yi[i],obj.location.z
    for i, obj in enumerate(target_collection.objects):
        obj.location = xi[i],yi[i],obj.location.z
        """

def object_rotation_new(collection: bpy.types.Collection):
    """Foreach object in collection define object's rotation"""
    number_of_objects = len(collection.objects)
    target_collection = bpy.data.collections[collection.name + "_copy"]
    for i in range(number_of_objects):
        rz = math.radians(random.randint(0, 359))
        collection.objects[i].rotation_euler[2] = rz
        target_collection.objects[i].rotation_euler[2] = rz

def render(number_of_positions: int, max_offset: int, around: int, output_dir: str, collection: bpy.types.Collection, environment_strength: float):
    """Position, rotate object and make render"""
    scene = bpy.context.scene
    number_of_objects = len(collection.objects)
    target_collection = bpy.data.collections[collection.name + "_copy"]
    ...
    for pos in range(number_of_positions):
        # Set objects location and rotation
        object_location_new(collection, max_offset)
        object_rotation_new(collection)
        
        for i in range(around):
            # Calculate angle in radians and set objects z-rotation
            angle = math.radians(360 / around)
            for i in range(number_of_objects):
                collection.objects[i].rotation_euler[2] += angle
                target_collection.objects[i].rotation_euler[2] += angle
...
class WM_OT_ExecuteButton(Operator):
    ...
    def execute(self, context):
        ...
        make_duplicate_collection(mytool.the_chosen_collection)
        #make_duplicate_objects(mytool.the_chosen_collection)
        create_materials(mytool.the_chosen_collection)
        materialize_duplicated_objects(mytool.the_chosen_collection)
        delete_and_hide_stuff()
        setup_shadow_catcher()
        setup_background()
        render(
